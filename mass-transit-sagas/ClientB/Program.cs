...
namespace ClientB
{
        ...
        IConsumer<OrderCanceled>
    {
        ...
        {
            if (context.Message.ClientLogin != "ClientB")
            {
                return;
            }

            ConsoleHelper.WriteLocked(ConsoleColor.Cyan,
                                        $"[ClientB] Do you confirm order #{context.Message.Id} with quantity {context.Message.Quantity}? (y/n)");
                ...
                if (Console.KeyAvailable)
                {
                    ...
                    if (key.Key == ConsoleKey.Y)
                    {
                        ...
                        ConsoleHelper.WriteLocked(ConsoleColor.Green,
                            $"[ClientB] Confirmed order #{context.Message.Id}");
                        break;
                    }
                    else if (key.Key == ConsoleKey.N)
                    {
                        ...
                        ConsoleHelper.WriteLocked(ConsoleColor.Red,
                            $"[ClientB] Order #{context.Message.Id} not confirmed");
                        break;
                    }
                }
                else
        ...
        public Task Consume(ConsumeContext<OrderAccepted> context)
        {
            ...
            ConsoleHelper.WriteLocked(ConsoleColor.Green,
                                        $"[ClientB] Order #{context.Message.Id} ACCEPTED by shop for quantity {context.Message.Quantity}. \n Send new order with 's' key");
        ...
        public Task Consume(ConsumeContext<OrderCanceled> context)
        {
            ...
            ConsoleHelper.WriteLocked(ConsoleColor.Red,
                                        $"[ClientB] Order #{context.Message.Id} CANCELED by shop for quantity {context.Message.Quantity}. \n Send new order with 's' key");
            _orderState.IsOrderInProcess = false;
            return Task.CompletedTask;
        }
    }
    ...
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            Console.WriteLine("ClientB started | Press 's' to send order, 'q' to quit...");
            while (true)
            {
                if (!clientOrderState.IsOrderInProcess)
                {
                    ...
                    if (key.Key == ConsoleKey.S)
                    {
                        ...
                        ConsoleHelper.WriteLocked(ConsoleColor.Cyan,
                                        $"[ClientB] Order with quantity {quantity} sent");
                    }
